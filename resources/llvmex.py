...
class KaleidoscopeEvaluator(object):
    ...
    def evaluate(self, codestr, optimize=True, llvmdump=False, machinedump=False):
        ...
        e.evaluate('''
            def foo(x y z)
                var s1 = x + y, s2 = z + y in
                    s1 * s2
            ''')
        self.assertEqual(e.evaluate('foo(1, 2, 3)', llvmdump=True), 15)
        # self.assertEqual(e.evaluate('', llvmdump=True), 15)
